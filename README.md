...
```shell
$ cd /opt/
$ wget https://github.com/gmellini/snort-rules-customization/archive/master.zip
$ unzip master.zip
...
I created a test/ folder and a shell script (test.it) that execute the program in test mode (all the needed files are inside the test dir). The test.it script execute following command: ./build_advanced_rules.pl -c ./pulledpork.conf -d ./downloaded.rules -t

```shell
$ ./test.it 
Running command: [/opt/snort-scripts/test/../build_advanced_rules.pl -c /opt/snort-scripts/test/pulledpork.conf -d /opt/snort-scripts/test/downloaded.rules -t]
----------------------------------------------------------------
=================
=== TEST MODE ===
=================
...
- write the local.advanced.rules in /tmp. Here you can find the new rules based on custom the regular expressions according to the needs of each user and defined inside the script
...
- '$HOME_NET any ' with '[192.168.0.1] [443,5555] ' ==> instead HOME_NET we match specific destination IP and ports
- '(msg:"' with '(msg:"NEW RULE - ' ==> rename the original rule prepending "NEW RULE" word

As an example follow the new rule created for the matching "ET TOR Known Tor Relay/Router (Not Exit) Node TCP Traffic group 654" that match the 'regexp' key. 
In the test case you can find the new rule in /tmp/local.advanced.rules, after the new rule you can find the original one commented:
...
Also in the downloaded.rules file the original rule is commented.
In the test case is the temp file /tmp/downloaded.rules.modified that has the following line:
...
#########################################
su - $PULLEDPORK_USER -c "/opt/snort-scripts/build_advanced_rules.pl -c /etc/nsm/pulledpork/pulledpork.conf"
```
